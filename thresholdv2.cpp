...
void weigths(int **W, int *x0, int *x1, int *x2, int *x3, int *x4, int *x5,int *x6, int *x7,int *x8, int *x9, int N) //pesos de todos los valores de entrada
{
...
void mul(int **W, int *s, int *h, int N) //pesos multiplicando los pesos de arriba por los de entrada
{ 
...
int check(int *v1, int *v2, int N) //retorna formateados para imprimier
{
...
int readfile(string file, int *v1, int col, int N)
{
   ...
   for (int i=0; i<N; i++) // cambia los valores 0 por -1
   { 
     if (v1[i] == 0)
       v1[i] = -1;
     if (((i+1)%col) == 0 )
...
int main(void)
{
  ...
  cout<<"----------CALCULATING WEIGTHS----------" <<endl;
  weigths(W, x0, x1, x2, x3,x4,x5,x6,x7,x8,x9, N);
  printw(W, N);

  int *s = new int[N]; 

  cout<<"----------PATTERN ENTERED----------" <<endl;
  readfile("entrada.txt", s,  col, N);
...
cout<<"----------MATCH FOUND----------"<<endl;
    for (int i=0; i<N; i++) //Este for imprime el patron mas cercano
    {
      if (s[i] == -1)
        cout << "0" << " ";
      else
  ...
  E = energy(W,s,N);
  cout << "-----------ENERGY OF END ITERATION: " << E<< "----------" << endl;
